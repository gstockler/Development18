...
%%% Labor process: assuming highly positive/negative correlation with its
...
for n = 1:N
    ...
    log_uh = normrnd(0,sigma_uh,1); 
    zh(n,1) = exp(-sigma_u./2)*exp(log_uh);
...
W =W_c-W_h;
%% 2) Welfare of NO C&H (det + stoc season) + risk
...
for n = 1:N
    for t = 1:T
        for m = 1:M
            ...
            c_ns(m,t,n) = z(n,1)*exp(-sigma_eps./2)*exp(log_eps(t));
            h_ns(m,t,n) = zh(n,1)*exp(-sigma_eps./2)*exp(log_eps(t));
